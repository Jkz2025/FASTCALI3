...
const CreatePedidos = () => {
  ...
  {/*================================================= CONSTANTE PARA EL PRODCUTO DE LAS LISTAS DE PRECIO ================================================= */ }
  ...
  async function createPedido(e) {
    ...
    try {
      ...
      const materialIds = materialesAgregados.map(m => m.material_id).join(',');
      const materialNames = materialesAgregados.map(m => m.material_name).join(',');
      const materialPrecios = materialesAgregados.map(m => m.precio_venta).join(',');
      const materialCantidad = materialesAgregados.map(m => m.cantidad).join(',');
      const materialDescuento = materialesAgregados.map(m => m.descuento).join(',');
      const porcentajeDescuento = materialesAgregados.map(m => m.descuento_porcentaje).join(',')
      ...
      if (!pedidoBodega) {
        ...
        for (let material of materialesAgregados) {
          const { material_id, material_name, precio_venta, cantidad, descuento, descuento_porcentaje, lista_precios, valorTotalProducto } = material;

          const { data, error: insertHistorialError } = await supabase.from('pedido_historial').insert({
            codigo_pedido: codigoPedido,
          ...
          if (insertHistorialError) {
            console.error('Error creating pedido historial:', insertHistorialError.message);
            alert('Error creating pedido historial: ' + insertHistorialError.message);
            return;
          }
        }
        // Insertar en la tabla 'pedidos'
        const { error: insertPedidoError } = await supabase.from('pedidos').insert({
          ...pedido,
          subtotal: subTotal,
          iva: valorIva,
          iva_porcentaje: 19,
        ...
        alert(`El Pedido ${codigoPedido} se creó satisfactoriamente`);
  ...
  const subTotal = materialesAgregados.reduce((acc, prod) => acc + prod.valorTotalProducto, 0);
  const descuento = materialesAgregados.reduce((acc, prod) => acc + prod.descuentoTotal, 0);
  ...
  const handleChangeBodega = (e) => {
    const selectedBodega = bodegas.find(b => b.id === parseInt(e.target.value, 10));
    if (selectedBodega) {
  ...
  const handleEmpleadoChange = (e) => {
    const selectedEmpleado = empleados.find(empleado => empleado.id === parseInt(e.target.value, 10));
  ...
  const handleIdAndName = (e) => {
    ...
    const listasParaMaterial = listasPrecioMateriales.filter((lista) => lista.material_id === selectedMaterial.material_id);
  ...
  useEffect(() => {
    const listaConDescuento = listasParaMaterial.map(l => l.descuento_porcentaje)
      ...
      <div className="bg-gradient-to-r from-gray-800 via-gray-900 to-black rounded-lg shadow-lg p-6">
        ...
        <form className="flex flex-col gap-4">
          ...
          <div className="bg-gradient-to-r from-gray-800 via-gray-900 to-black rounded-lg shadow-lg p-6">
            ...
            <div className="flex items-center gap-2 "> {/* Contenedor del select y la imagen */}
              <select name="bodega_id" value={pedido.bodega_id} onChange={handleChangeBodega} className="p-2 rounded-lg bg-gray-800 text-white">
                <option value="">Selecciona una bodega</option>
                {bodegas.map((bodega) => (
                  <option key={bodega.id} value={bodega.id}>{bodega.name}</option>
                ))}
              </select>
              <select name="empleado" value={pedido.empleado_id} onChange={handleEmpleadoChange} className="p-2 rounded-lg bg-gray-800 text-white">
                <option value="">Selecciona un Empleado</option>
                {empleados.map((empleado) => (
                  <option key={empleado.empleado_id} value={empleado.id}> {empleado.nombre} {empleado.apellido} - {empleado.id} </option>
                ))}
              </select>
              <img className='h-[40px] cursor-pointer' onClick={handleClickAgregarCliente} src={agregar} alt="" />
              <select name="user_id" value={pedido.user_id} onChange={handleUserChange} className="p-2 rounded-lg bg-gray-800 text-white">
                <option value="">Selecciona un cliente</option>
                {users.map((user) => (
                  <option key={user.user_id} value={user.id}>{user.name} {user.apellido} - {user.id}</option>
                ))}
              </select>
              ...
              <img className='h-[40px] cursor-pointer' onClick={handleClickAgregarCliente} src={agregar} alt="" />
                ...
                {/*================================================= SELECT FORMA PAGO   ================================================= */}
                <div className="flex items-center gap-2 py-4"> {/* Contenedor del select y la imagen */}
              <select name="forma_pago" value={pedido.forma_pago} onChange={handleFormaPago} className="p-2   rounded-lg bg-gray-800 text-white">
                <option value="" disabled selected>Selecciona Forma de Pago</option>
                <option value="Efectvio">Efectivo</option>
              ...
              >
                <option value="" disabled selected>Selecciona Medio Pago</option>
                <option value="Efectivo">Efectivo</option>
                <option value="Transferencia Debito">Transferencia Debito</option>
                <option value="Transferencia Credito">Transferencia Credito</option>
                <option value="Consignacion Bancaria">Consignacion Bancaria</option>
                <option value="Cheque">Cheque</option>
            ...
            <div className="max-w-4xl mx-auto ">
              <div className="bg-gradient-to-r from-gray-800 via-gray-900 to-black rounded-lg shadow-lg p-3">
                <table className="w-full table-auto bg-gray-900 rounded-lg text-white">
                  <thead>
                    <tr>
                      ...
                      <th className="px-4 py-2 text-center bg-gray-800">Precio</th>
                ...
                <div className="w-full flex flex-wrap justify-center py-4 gap-4 mt-4">
                  ...
                  >
                    <option value="" disabled>Escribe un Codigo</option>
                    {uniqueMaterials.map(material => (
                      <option key={material.material_id} value={material.material_id}>{material.material_id}</option>
                    ))}
                  </select>
                  <select
                    ...
                    onChange={handleIdAndName}
                  >
                    <option value="" disabled>Escribe un material</option>
                    {uniqueMaterials.map(material => (
                      <option key={material.material_name} value={material.material_name}>{material.material_name}</option>
                    ))}
                  </select>
                  <select
                    name="lista_precios"
            ...
            <tbody >
              {materialesAgregados.map((prod, index) => (
                <tr key={prod.material_id}>
                  ...
                  <td className="border px-4 py-2 text-center">{prod.descuentoTotal}</td>
                  <td className="border px-4 py-2">{prod.valorTotalProducto}</td>
                  <td>
          ...
          <form className='mt-2 py-5'>
            <p className="text-white">Codigo Pedido:  {codigoPedido}</p>
            <p className="text-white">Descuento:  {descuento.toLocaleString()}</p>
            <p className="text-white">Sub Total:  {subTotal.toLocaleString()}</p>
            <p className="text-white">IVA (19%): {valorIva.toLocaleString()}</p>
            <p className="text-white">Valor Total:  {valorTotal.toLocaleString()}</p>
            <p className="text-white">Fecha de Creación: {new Date(pedido.fecha_creacion).toLocaleString()}</p>
            <ButtonFastCali text='Crear Pedido' onClick={createPedido} />
          </form>
        </form>
      </div>
      <Link to='/FacturacionyPedidos/Pedidos'> <div className='py-4 flex justify-center'>
        <Button text='🔙' />
      </div>
      </Link>
